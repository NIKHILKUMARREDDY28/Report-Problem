from flask import render_template, request, url_for, flash, redirect, session, jsonify, make_response,Flask
...
def login():
    ...
    if request.method == "POST":
        if users.find_one({'email': request.form['email']}):
            if check_password_hash(users.find_one({'email': request.form['email']})['password'],
...
def register():
    ...
    if request.method == "POST":
        if users.find_one({'email': request.form['email']}):
            ...
            users.insert_one({'name': request.form['name'], 'phone_number': request.form['phonenumber'],
...
def addissue():
    if session.get('name'):
        if request.method == "POST":
            if issues.find_one(
                    {'title': request.form['title'], 'location': request.form['location'], 'date': request.form['date'],
                     'details': request.form['details'], 'name': session.get('name'), 'email': session.get('email')}):
                ...
                if grid_fs.find_one(file_id) is not None:
                    issues.insert_one({'title': request.form['title'], 'location': request.form['location'],
                                       'date': request.form['date'], 'details': request.form['details'],
                                       'name': session.get('name'), 'email': session.get('email'), 'status': status,
        ...
        if request.method == 'POST':
            ...
            if grid_fs.find_one(file_id) is not None:
                issues.find_one_and_update({'_id': ObjectId(idx)}, {
                    "$set": {'title': request.form['title'], 'location': request.form['location'],
                             'date': request.form['date'], 'details': request.form['details'],
                             'name': session.get('name'), 'email': session.get('email'), 'imageid': file_id,
                             'imagedata': imagedata}})
